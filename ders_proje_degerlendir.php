...
$degerbir_odev = "-1";
if (isset($_GET['projeno'])) {
  $degerbir_odev = $_GET['projeno'];
}
$degeruc_odev = "-1";
if (isset($_GET['subeno'])) {
  $degeruc_odev = $_GET['subeno'];
}
$degeriki_odev = "-1";
if (isset($_SESSION['MM_Username'])) {
...
mysql_select_db($database_lmscon, $lmscon);
$query_odev = sprintf("SELECT DISTINCT O.*, CONCAT(K.ad, ' ', K.soyad) AS adsoyad FROM lms_odev O, lms_proje P, lms_sube S, lms_ders D, lms_sinif SN, lms_kullanici K WHERE O.kimlik = K.kimlik AND O.projeno = P.no AND P.dersno = S.dersno AND S.no = SN.subeno AND O.kimlik = SN.kimlik AND O.projeno = %s AND S.kimlik = %s AND S.no = %s ORDER BY K.soyad", GetSQLValueString($degerbir_odev, "int"),GetSQLValueString($degeriki_odev, "text"),GetSQLValueString($degeruc_odev, "int"));
...
$degerbir_ortalama = "-1";
if (isset($_GET['projeno'])) {
  $degerbir_ortalama = $_GET['projeno'];
}
$degeruc_ortalama = "-1";
if (isset($_GET['subeno'])) {
  $degeruc_ortalama = $_GET['subeno'];
}
$degeriki_ortalama = "-1";
if (isset($_SESSION['MM_Username'])) {
  $degeriki_ortalama = $_SESSION['MM_Username'];
}
mysql_select_db($database_lmscon, $lmscon);
$query_ortalama = sprintf("SELECT ROUND(AVG(notu)) AS ortalama FROM lms_odev O, lms_proje P, lms_sube S, lms_ders D, lms_sinif SN, lms_kullanici K WHERE O.kimlik = K.kimlik AND O.projeno = P.no AND P.dersno = S.dersno AND S.no = SN.subeno AND O.kimlik = SN.kimlik AND O.projeno = %s AND S.kimlik = %s AND S.no = %s ORDER BY K.soyad", GetSQLValueString($degerbir_ortalama, "int"),GetSQLValueString($degeriki_ortalama, "text"),GetSQLValueString($degeruc_ortalama, "int"));
$ortalama = mysql_query($query_ortalama, $lmscon) or die(mysql_error());
$row_ortalama = mysql_fetch_assoc($ortalama);
$totalRows_ortalama = mysql_num_rows($ortalama);
...
?>
<?php
$_SESSION['dersno'] = $_GET['dersno'];
$_SESSION['subeno'] = $_GET['subeno'];
$_SESSION['projeno'] = $_GET['projeno'];
?>
...
<!-- fwtable fwsrc="lms_ic.png" fwpage="LMS ic" fwbase="lms_ic.gif" fwstyle="Dreamweaver" fwdocid = "52238024" fwnested="0" -->
  ...
  <tr>
   ...
   <td rowspan="2" class="ustbaslik"><!-- InstanceBeginEditable name="Baslik" -->
     <table width="100%" border="0" cellspacing="5" cellpadding="5">
       <tr>
         <td width="7%"><a href="ders.php?dersno=<?php echo $_SESSION['dersno']."&"."subeno=".$_SESSION['subeno']; ?>"><img src="simgeler/back_24.gif" alt="Geri dön" width="24" height="24" border="0" /></a></td>
  ...
  <tr>
   ...
   <td colspan="19" valign="top"><!-- InstanceBeginEditable name="İcerik" -->
     <table width="100%" border="0" cellspacing="5" cellpadding="5">
       <tr>
         <td width="60%" class="baslik">Gönderilmiş Olan Ödevler &amp; Projeler</td>
         <td class="baslik">Ödev &amp; Proje Ayrıntı</td>
       </tr>
       <tr>
         <td width="60%" valign="top"><table width="100%" border="0" cellspacing="5" cellpadding="5">
           <?php if ($totalRows_odev == 0) { // Show if recordset empty ?>
             <tr>
               <td class="icyazi">Gönderilmiş ödev / proje bulunmamaktadır.</td>
             </tr>
           ...
           <?php if ($totalRows_odev > 0) { // Show if recordset not empty ?>
             <table width="100%" border="0" cellspacing="2" cellpadding="3">
               <tr>
                 <td width="17%" nowrap="nowrap" class="icyazi"><div align="center"><strong>Öğrenci No</strong></div></td>
                  <td width="21%" nowrap="nowrap" class="icyazi"><strong>Ad - Soyad</strong></td>
                  <td width="30%" nowrap="nowrap" class="icyazi"><div align="center"><strong>İlk Gönderilme Tarihi</strong></div></td>
                  <td nowrap="nowrap" class="icyazi"><div align="center"><strong>Not</strong></div></td>
                  <td colspan="2" nowrap="nowrap" class="icyazi"><div align="center"><strong>İşlem</strong></div></td>
               </tr>
               <tr>
               ...
               <?php do { ?>
                 <tr>
                   <td class="icyazi"><div align="center"><?php echo $row_odev['kimlik']; ?></div></td>
                   <td class="icyazi"><?php echo $row_odev['adsoyad']; ?></td>
                   <td class="icyazi"><div align="center"><?php echo $row_odev['guncelleme']; ?></div></td>
                   <td width="10%" class="icyazi"><div align="center"><strong><?php echo $row_odev['notu']; ?></strong></div></td>
                   <td width="1%" class="icyazi"><div align="center"><a href="ders_proje_degerlendir2.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."odevno=".$row_odev['no'] ?>"><img src="simgeler/edit_16.gif" title="Öğrenciyi değerlendir / dönüt yaz" width="16" height="16" border="0" /></a></div></td>
                   <td width="1%" class="icyazi"><div align="center"><a href="ders_proje_dokuman_sil.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."odevno=".$row_odev['no'] ?>"><img src="simgeler/del_16.gif" title="Sil" width="16" height="16" border="0" onclick="GP_popupConfirmMsg('G&ouml;nderilmi&#351; olan &ouml;dev / proje sistemden tamamen silinecektir.');return document.MM_returnValue" /></a><a href="ders_proje_dokuman_sil.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."odevno=".$row_odev['no'] ?>"></a></div></td>
                 </tr>
                 <?php } while ($row_odev = mysql_fetch_assoc($odev)); ?>
              </table>
         ...
         <td valign="top"><table width="100%" border="0" cellspacing="2" cellpadding="3">
           ...
           <tr>
             <td colspan="3" valign="top" class="icyazi">Gönderilen Ödev / Proje Sayısı: <strong><?php echo $totalRows_odev ?></strong> </td>
             </tr>
           <tr>
             <td colspan="3" valign="top" nowrap="nowrap" class="icyazi">Sınıf Not Ortalaması: <strong><?php echo $row_ortalama['ortalama']; ?></strong></td>
             </tr>
         </table></td>
       </tr>
     </table>
