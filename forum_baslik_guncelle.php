...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE lms_forum_baslik SET baslik=%s, aciklama=%s, kilit=%s WHERE `no`=%s",
                       GetSQLValueString($_POST['baslik'], "text"),
                       GetSQLValueString($_POST['aciklama'], "text"),
                       GetSQLValueString($_POST['kilit'], "text"),
...
if (isset($_GET['baslikno'])) {
  $degeriki_ders = $_GET['baslikno'];
}
mysql_select_db($database_lmscon, $lmscon);
$query_ders = sprintf("SELECT * FROM lms_forum_baslik WHERE kimlik = %s AND no = %s", GetSQLValueString($deger_ders, "text"),GetSQLValueString($degeriki_ders, "int"));
...
mysql_select_db($database_lmscon, $lmscon);
$query_baslik = sprintf("SELECT CONCAT(DE.kod, ' - ', SU.kod, '  ', DE.ad) AS anabaslik, DE.no AS dersno, SU.no AS subeno FROM lms_ders DE, lms_sube SU WHERE DE.no = SU.dersno AND SU.kimlik = %s AND DE.no = %s AND SU.no = %s", GetSQLValueString($deger_baslik, "text"),GetSQLValueString($degeriki_baslik, "int"),GetSQLValueString($degeruc_baslik, "int"));
...
<!-- fwtable fwsrc="lms_ic.png" fwpage="LMS ic" fwbase="lms_ic.gif" fwstyle="Dreamweaver" fwdocid = "52238024" fwnested="0" -->
  ...
  <tr>
   ...
   <td rowspan="2" class="ustbaslik"><!-- InstanceBeginEditable name="Baslik" -->
     <table width="100%" border="0" cellspacing="5" cellpadding="5">
       <tr>
         <td width="3%" valign="top"><a href="forumlar.php"><img src="simgeler/back_24.gif" alt="Geri Dön" width="24" height="24" border="0" /></a></td>
         <td width="97%" valign="top"><?php echo $row_baslik['anabaslik']; ?></td>
       </tr>
     </table>
  ...
  <tr>
   ...
   <td colspan="19" valign="top"><!-- InstanceBeginEditable name="İcerik" -->
     <table width="100%" border="0" cellspacing="5" cellpadding="5">
       <tr>
         <td width="60%" class="baslik">Forum Başlığını Güncelle</td>
         <td class="baslik">&nbsp;</td>
        </tr>
       <tr>
         <td valign="top"><form action="<?php echo $editFormAction; ?>" id="form1" name="form1" method="POST">
           <table width="100%" border="0" cellspacing="2" cellpadding="3">
             <tr>
               <td width="20%" class="icyazi"><strong>Başlık</strong></td>
               <td width="3%" class="icyazi"><strong>:</strong></td>
               <td width="77%"><label>
                 <input name="baslik" type="text" class="metinkutusu" id="baslik" value="<?php echo $row_ders['baslik']; ?>" size="50" maxlength="255" />
                 <span class="baslik">*</span></label></td>
             </tr>
             <tr>
               <td valign="top" class="icyazi"><strong>Açıklama</strong></td>
               <td valign="top" class="icyazi"><strong>:</strong></td>
               <td><label>
                 <textarea name="aciklama" cols="60" rows="7" class="metinkutusu" id="aciklama"><?php echo $row_ders['aciklama']; ?></textarea>
                 <span class="baslik">*</span></label></td>
             </tr>
             <tr>
               <td class="icyazi"><strong>Başlığı Kilitle</strong></td>
               <td class="icyazi"><strong>:</strong></td>
               <td><label>
                 <select name="kilit" id="kilit">
                   <option value="<?php echo $row_ders['kilit']; ?>" selected="selected"><?php echo $row_ders['kilit']; ?></option>
                   <option value="HAYIR">----------</option>
                   <option value="EVET">EVET</option>
                   <option value="HAYIR">HAYIR</option>
                 </select>
                 <span class="baslikMAVI">**</span></label></td>
             </tr>
             <tr>
               <td><input name="no" type="hidden" id="no" value="<?php echo $row_ders['no']; ?>" /></td>
               <td>&nbsp;</td>
               <td><label>
                 <input name="button" type="submit" class="buton" id="button" onclick="YY_checkform('form1','baslik','#q','0','Başlık','aciklama','1','1','Açıklama');return document.MM_returnValue" value="Başlığı Güncelle" />
               </label></td>
             </tr>
             <tr>
               <td colspan="2"><div align="right"> <span class="baslik">*</span></div></td>
               <td class="icyazi">Doldurulması zorunlu alanlar</td>
             </tr>
             <tr>
               <td colspan="2" valign="top"><div align="right"><span class="baslikMAVI">**</span></div></td>
               <td class="icyazi">Kilitlenen başlık altındaki yorumlar okunabilir ancak yeni yorum yapılamaz.</td>
             </tr>
           </table>
