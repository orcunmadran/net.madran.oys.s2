...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE lms_forum_yorum SET yorum=%s WHERE `no`=%s",
                       GetSQLValueString($_POST['yorum']."\n"."\n"." (Bu yorum ".date("d.m.Y - H:i")." tarihinde güncellendi.)", "text"),
...
if (isset($_GET['yorumno'])) {
  $deger_fy = $_GET['yorumno'];
}
$degeriki_fy = "-1";
if (isset($_SESSION['MM_Username'])) {
  $degeriki_fy = $_SESSION['MM_Username'];
}
mysql_select_db($database_lmscon, $lmscon);
$query_fy = sprintf("SELECT * FROM lms_forum_yorum WHERE no = %s AND kimlik = %s", GetSQLValueString($deger_fy, "int"),GetSQLValueString($degeriki_fy, "text"));
...
<!-- fwtable fwsrc="lms_ic.png" fwpage="LMS ic" fwbase="lms_ic.gif" fwstyle="Dreamweaver" fwdocid = "52238024" fwnested="0" -->
  ...
  <tr>
   ...
   <td colspan="19" valign="top"><!-- InstanceBeginEditable name="İcerik" -->
     <table width="100%" border="0" cellspacing="5" cellpadding="5">
       <tr>
         <td width="60%" valign="top"><form action="<?php echo $editFormAction; ?>" id="form1" name="form1" method="POST">
           <table width="100%" border="0" cellspacing="2" cellpadding="3">
             <tr>
               <td class="baslik">Yorum Güncelle</td>
                </tr>
             <tr>
               <td><hr size="1" /></td>
                </tr>
             <tr>
               <td><label>
                 <textarea name="yorum" cols="80" rows="14" class="metinkutusu" id="yorum"><?php echo $row_fy['yorum']; ?></textarea>
                 </label></td>
                </tr>
             <tr>
               <td><label>
                 <input name="button" type="submit" class="buton" id="button" onclick="YY_checkform('form1','yorum','1','1','Yorum','no','#q','0','Yorum NO');return document.MM_returnValue" value="Yorumu Güncelle" />
                 <input name="no" type="hidden" id="no" value="<?php echo $row_fy['no']; ?>" />
               </label></td>
                </tr>
             </table>
