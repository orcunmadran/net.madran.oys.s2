...
mysql_select_db($database_lmscon, $lmscon);
$query_ders = sprintf("SELECT DISTINCT DE.*, SU.no AS subeno, SU.kod AS subekodu, CONCAT(K.ad, ' ', K.soyad) AS adsoyad, SU.kimlik AS sorumlu FROM lms_ders DE, lms_sube SU, lms_sinif SN, lms_kullanici K WHERE DE.no = SU.dersno AND SU.no = SN.subeno AND SU.kimlik = K.kimlik AND SN.kimlik = %s AND DE.aktif = 'EVET' ORDER BY DE.kod", GetSQLValueString($deger_ders, "text"));
...
$deger_subeler = "-1";
if (isset($_SESSION['MM_Username'])) {
  $deger_subeler = $_SESSION['MM_Username'];
}
mysql_select_db($database_lmscon, $lmscon);
$query_subeler = sprintf("SELECT DISTINCT DE.*, SU.no AS subeno, SU.kod AS subekodu, CONCAT(K.ad, ' ', K.soyad) AS adsoyad, SU.kimlik AS sorumlu FROM lms_ders DE, lms_sube SU, lms_kullanici K WHERE DE.no = SU.dersno AND SU.kimlik = %s AND SU.kimlik = K.kimlik AND DE.aktif = 'EVET' ORDER BY DE.kod", GetSQLValueString($deger_subeler, "text"));
$subeler = mysql_query($query_subeler, $lmscon) or die(mysql_error());
$row_subeler = mysql_fetch_assoc($subeler);
$totalRows_subeler = mysql_num_rows($subeler);
  ...
  <tr>
   ...
   <td rowspan="2" class="ustbaslik"><!-- InstanceBeginEditable name="Baslik" -->
     <table width="100%" border="0" cellspacing="5" cellpadding="5">
       <tr>
         <td>Tartışma Grupları (Forumlar)</td>
       </tr>
     </table>
     ...
     <table width="100%" border="0" cellspacing="1" cellpadding="1">
       ...
       <tr>
         <td><table border="0" cellspacing="5" cellpadding="5">
           <?php if ($totalRows_ders == 0) { // Show if recordset empty ?>
             <tr>
               <td colspan="5"><table width="100%" border="0" cellspacing="2" cellpadding="3">
                      <tr>
                        <td width="4%"><img src="simgeler/about_24.gif" width="24" height="24" /></td>
                        <td width="96%" class="arabaslik">Sistem Mesajı</td>
                      </tr>
                      <tr>
                        <td>&nbsp;</td>
                        <td nowrap="nowrap" class="icyazi">Sistem içinde katılabileceğiniz tartışma grubu bulunmamaktadır.</td>
                      </tr>
			...
			<?php if ($totalRows_ders > 0) { ?>
             <tr>
             <td nowrap="nowrap" bgcolor="#F1F1F1" class="arabaslik"><div align="center">Ders Kodu</div></td>
             <td nowrap="nowrap" bgcolor="#F1F1F1" class="arabaslik"><div align="center">Şube</div></td>
             <td nowrap="nowrap" bgcolor="#F1F1F1" class="arabaslik">Forum Adı</td>
             <td nowrap="nowrap" bgcolor="#F1F1F1" class="arabaslik">Forum Sorumlusu</td>
             <td nowrap="nowrap" class="arabaslik">&nbsp;</td>
           </tr>
           <?php do { ?>
             <tr>
               <td nowrap="nowrap" class="icyazi"><div align="center"><?php echo $row_ders['kod']; ?></div></td>
               <td nowrap="nowrap" class="icyazi"><div align="center"><?php echo $row_ders['subekodu']; ?></div></td>
               <td nowrap="nowrap" class="icyazi"><a href="forum2.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."dersno=".$row_ders['no']."&"."subeno=".$row_ders['subeno'] ?>"><?php echo $row_ders['ad']; ?></a></td>
               <td nowrap="nowrap" class="icyazi"><a href="profil.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."kimlik=".$row_ders['sorumlu'] ?>"><?php echo $row_ders['adsoyad']; ?></a></td>
               <td nowrap="nowrap" class="icyazi">&nbsp;</td>
             </tr>
             <?php } while ($row_ders = mysql_fetch_assoc($ders)); ?>
			<?php } ?>
         </table></td>
        </tr>
        <?php }?>
      ...
      <tr>
         <td><table border="0" cellspacing="5" cellpadding="5">
           <?php if ($totalRows_subeler == 0) { // Show if recordset empty ?>
             <tr>
               <td colspan="5"><table width="100%" border="0" cellspacing="2" cellpadding="3">
                      <tr>
                        <td width="4%"><img src="simgeler/about_24.gif" width="24" height="24" /></td>
           ...
           <?php do { ?>
             <tr>
               <td nowrap="nowrap" class="icyazi"><div align="center"><?php echo $row_subeler['kod']; ?></div></td>
               <td nowrap="nowrap" class="icyazi"><div align="center"><?php echo $row_subeler['subekodu']; ?></div></td>
               <td nowrap="nowrap" class="icyazi"><a href="forum.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."dersno=".$row_subeler['no']."&"."subeno=".$row_subeler['subeno'] ?>"><?php echo $row_subeler['ad']; ?></a></td>
               <td nowrap="nowrap" class="icyazi"><a href="profil.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."kimlik=".$row_subeler['sorumlu'] ?>"><?php echo $row_subeler['adsoyad']; ?></a></td>
               <td nowrap="nowrap" class="icyazi">&nbsp;</td>
             </tr>
             <?php } while ($row_subeler = mysql_fetch_assoc($subeler)); ?>
			<?php } ?>
         </table></td>
        </tr>
        <?php }?>
