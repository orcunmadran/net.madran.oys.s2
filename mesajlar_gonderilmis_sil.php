...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "islem")) {
  ...
  $updateGoTo = "mesaj_gonderilen.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_lmscon, $lmscon);
$query_m = sprintf("SELECT no, silindi FROM lms_mesaj WHERE no = %s AND gonderen = %s", GetSQLValueString($degerbir_m, "int"),GetSQLValueString($degeriki_m, "text"));
