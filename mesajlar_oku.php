...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "islem")) {
  $updateSQL = sprintf("UPDATE lms_mesaj SET okundu=%s WHERE `no`=%s",
                       GetSQLValueString($_POST['okundu'], "text"),
...
mysql_select_db($database_lmscon, $lmscon);
$query_m = sprintf("SELECT no, okundu FROM lms_mesaj WHERE no = %s AND alici LIKE %s", GetSQLValueString($degerbir_m, "int"),GetSQLValueString("%" . $degeriki_m . "%", "text"));
...
<form id="islem" name="islem" method="POST" action="<?php echo $editFormAction; ?>">
  ...
  <input name="okundu" type="hidden" id="okundu" value="<?php echo $row_m['okundu']." ".$row_tanitim['kimlik']; ?>" />
