...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO lms_not_defteri (kimlik, konu, icerik, hatirlat, nezaman) VALUES (%s, %s, %s, %s, %s)",
                       GetSQLValueString($_POST['kimlik'], "text"),
                       GetSQLValueString($_POST['konu'], "text"),
                       GetSQLValueString($_POST['icerik'], "text"),
                       GetSQLValueString($_POST['hatirlat'], "text"),
                       GetSQLValueString($_POST['nezaman'], "date"));

  mysql_select_db($database_lmscon, $lmscon);
  $Result1 = mysql_query($insertSQL, $lmscon) or die(mysql_error());

  $insertGoTo = "not_defteri.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_lmscon, $lmscon);
$query_nd = sprintf("SELECT *, DATE_FORMAT(tarih, '%%d.%%m.%%Y - %%H:%%i') AS tarihsaat FROM lms_not_defteri WHERE kimlik = %s ORDER BY tarih DESC", GetSQLValueString($bir_nd, "text"));
...
//copyright (c)1998,2002 Yaromat.com
  ...
  if (myErr!=''){alert('Bilgi formunda eksik / hatalı alanlar var:\t\t\t\t\t\n\n'+myErr)}
...
<!-- fwtable fwsrc="lms_ic.png" fwpage="LMS ic" fwbase="lms_ic.gif" fwstyle="Dreamweaver" fwdocid = "52238024" fwnested="0" -->
  ...
  <tr>
   ...
   <td colspan="19" valign="top"><!-- InstanceBeginEditable name="İcerik" -->
     <table width="100%" border="0" cellspacing="5" cellpadding="5">
       <tr>
         <td width="60%" class="baslik">Almış Olduğunuz Notlar</td>
         <td class="baslik">Yeni Not Al</td>
       </tr>
       <tr>
         <td valign="top"><?php if ($totalRows_nd == 0) { // Show if recordset empty ?>
             <table width="100%" border="0" cellspacing="2" cellpadding="3">
               <tr>
                 <td><p class="arabaslik">Not defterine kaydettiğiniz bir notunuz bulunmamaktadır.</p>
                  <p class="icyazi">Yeni bir not almak için sağ tarafdaki formu kullanabilirsiniz. Notunuza alarm eklediğinizde belirlediğiniz gün içerisinde notunuz ile ilgili hatırlatma &quot;<strong>Anasayfa</strong>&quot;'da yer alacaktır.</p></td>
               </tr>
             </table>
           ...
           <?php if ($totalRows_nd > 0) { // Show if recordset not empty ?>
             <table width="100%" border="0" cellspacing="5" cellpadding="5">
               <tr>
                 <td width="19%" nowrap="nowrap" bgcolor="#F1F1F1" class="arabaslik"><div align="center">Not Kayıt Tarihi</div></td>
                 <td colspan="2" bgcolor="#F1F1F1" class="arabaslik">Konu</td>
                 <td colspan="2" nowrap="nowrap" bgcolor="#F1F1F1" class="arabaslik"><div align="center">İşlemler</div></td>
               </tr>
               <?php do { ?>
                 <tr>
                   <td valign="top" nowrap="nowrap" class="icyazi"><?php echo $row_nd['tarihsaat']; ?></td>
                   <td width="60%" valign="top" class="icyazi"><a href="not_defteri_goster.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."notno=".$row_nd['no'] ?>"><?php echo $row_nd['konu']; ?></a></td>
                   <td width="6%" valign="top" class="icyazi"><div align="center"><a href="not_defteri_goster.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."notno=".$row_nd['no'] ?>"><img src="simgeler/notep_16.gif" alt="<?php echo "Not Önizleme: ".$row_nd['icerik']; ?>" width="16" height="16" border="0" /></a></div></td>
                   <td width="1%" valign="top" class="icyazi"><div align="right"><a href="not_defteri_sil.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."notno=".$row_nd['no'] ?>"><img src="simgeler/del_16.gif" alt="Sil" width="16" height="16" border="0" onclick="GP_popupConfirmMsg('Alm&#305;&#351; oldu&#287;unuz not sistemden tamamen silinecektir!');return document.MM_returnValue" /></a></div></td>
                   <td width="1%" valign="top" class="icyazi"><div align="left"><a href="not_defteri_guncelle.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."notno=".$row_nd['no'] ?>"><img src="simgeler/edit_16.gif" alt="G&uuml;ncelle" width="16" height="16" border="0" /></a></div></td>
                  </tr>
                 <?php } while ($row_nd = mysql_fetch_assoc($nd)); ?>
             </table>
             <?php } // Show if recordset not empty ?></td>
         <td valign="top"><form id="form1" name="form1" method="POST" action="<?php echo $editFormAction; ?>">
           <table width="100%" border="0" cellspacing="2" cellpadding="3">
             <tr>
               <td width="13%" height="30" class="icyazi"><strong>Konu</strong></td>
               <td width="3%" class="icyazi"><strong>:</strong></td>
               <td colspan="2" class="icyazi"><label>
                 <input name="konu" type="text" class="metinkutusu" id="konu" size="35" />
                 <span class="baslik">*</span></label></td>
             </tr>
             <tr>
               <td valign="top" class="icyazi"><strong>İçerik</strong></td>
               <td valign="top" class="icyazi"><strong>:</strong></td>
               <td colspan="2" class="icyazi"><label>
                 <textarea name="icerik" cols="40" rows="5" class="metinkutusu" id="icerik"></textarea>
                 <span class="baslik">*</span></label></td>
             </tr>
             <tr>
               <td class="icyazi"><div align="right"><img src="simgeler/alarm_16.gif" width="16" height="16" /></div></td>
               <td class="icyazi"><strong>:</strong></td>
               <td width="8%" class="icyazi"><label>
                 <input name="hatirlat" type="checkbox" class="icyazi" id="hatirlat" value="Evet" />
               </label></td>
               <td class="icyazi">Aldığım notu bana hatırlat</td>
               </tr>
             <tr>
               <td class="icyazi"><div align="right"><img src="simgeler/cal_16.gif" width="16" height="16" /></div></td>
               <td class="icyazi"><strong>:</strong></td>
               <td colspan="2" class="icyazi"><label>
                 <input name="nezaman" type="text" class="metinkutusu" id="nezaman" onclick="GetDate(this);" value="Tıkla, sol-üst köşeden tarihi belirle" size="32" readonly="readonly" />
               </label></td>
               </tr>
             <tr>
               <td class="icyazi">&nbsp;</td>
               <td class="icyazi">&nbsp;</td>
               <td colspan="2" class="icyazi"><label>
                 <input name="button" type="submit" class="buton" id="button" onclick="YY_checkform('form1','konu','#q','0','Konu','icerik','1','1','İçerik');return document.MM_returnValue" value="Not Al" />
