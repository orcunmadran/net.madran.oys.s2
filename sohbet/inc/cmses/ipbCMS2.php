...
class IPBCMS {
	...
	function IPBCMS() {
		...
		$this->autoLogStmt = new Statement("SELECT id FROM {$GLOBALS['INFO']['sql_tbl_prefix']}members WHERE id=? AND member_login_key=? LIMIT 1");
		...
		if (isset($_COOKIE['member_id']) && isset($_COOKIE['pass_hash']))
		{
			...
			$pid = substr( $_COOKIE['pass_hash'], 0, 32 );

			$rs = $this->autoLogStmt->process($mid,$pid);
			$rec = $rs->next();
