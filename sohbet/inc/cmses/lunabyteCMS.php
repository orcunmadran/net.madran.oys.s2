...
class LBCMS {
  ...
  function LBCMS() {
    ...
    $this->loginStmt = new Statement("SELECT ID_MEMBER as id, memberName AS login, passwd from {$GLOBALS['db_prefix']}members WHERE memberName=? AND passwd=? LIMIT 1");
    $this->getUserStmt = new Statement("SELECT ID_MEMBER AS id, memberName AS login, memberGroup as status FROM {$GLOBALS['db_prefix']}members WHERE ID_MEMBER=? LIMIT 1");
