...
mysql_select_db($database_lmscon, $lmscon);
$query_ders = sprintf("SELECT no, kod FROM lms_ders WHERE no = %s AND kimlik = %s ", GetSQLValueString($colname_ders, "int"),GetSQLValueString($deger_ders, "text"));
...
mysql_select_db($database_lmscon, $lmscon);
$query_prj = sprintf("SELECT * FROM lms_proje WHERE dersno = %s ORDER BY orjinalad", GetSQLValueString($colname_prj, "int"));
  ...
  <tr>
   ...
   <td rowspan="2" class="ustbaslik"><!-- InstanceBeginEditable name="Baslik" -->
     <table width="100%" border="0" cellspacing="5" cellpadding="5">
       <tr>
         <td width="7%"><a href="yonetim_dersler.php"><img src="simgeler/back_24.gif" alt="Yönetim paneline geri dön" width="24" height="24" border="0" /></a></td>
  ...
  <tr>
   ...
   <td colspan="19" valign="top"><!-- InstanceBeginEditable name="İcerik" -->
     <table width="100%" border="0" cellspacing="1" cellpadding="1">
       <tr>
         <td width="60%" valign="top"><form action="yonetim_ders_proje2.php" method="POST" enctype="multipart/form-data" name="form1" id="form1">
           <table width="500" border="0" cellpadding="3" cellspacing="2">
             <tr>
               <td colspan="3" class="baslik">Ödev &amp; Proje  Yükle / Güncelle</td>
               </tr>
             <tr>
               <td colspan="3" class="icyazi"><hr size="1" /></td>
               </tr>
             <tr>
               <td width="99" nowrap="nowrap" class="icyazi"><span class="icyazi"><strong>Dokümanı Seç</strong></span></td>
             ...
             <tr>
               <td class="icyazi"><strong>Teslim Tarihi</strong></td>
               <td class="icyazi"><strong>:</strong></td>
               <td>                 <input name="teslim" type="text" class="metinkutusu" id="teslim" onclick="GetDate(this);" size="15" readonly="readonly" />               </td>
             </tr>
             <tr>
               <td><input name="no" type="hidden" id="no" value="<?php echo $row_ders['no']; ?>" /></td>
               <td>&nbsp;</td>
               <td><input name="button" type="submit" class="buton" id="button" onclick="YY_checkform('form1','teslim','#q','0','Teslim Tarihi');return document.MM_returnValue" value="Doküman Yükle" /></td>
             </tr>
             <tr>
           ...
           <table width="500" border="0" cellpadding="3" cellspacing="2">
             <tr>
               <td width="3%" valign="top" class="baslikMAVI">*</td>
               <td width="97%"><p class="icyazi">Dosya adı, sistem içerisinde ödev / proje adı olarak kullanılacaktır.</p>                  </td>
             </tr>
             <tr>
               <td valign="top"><span class="baslikMAVI">*</span></td>
               <td valign="top" class="icyazi">Yüklemiş olduğunuz bir ödev / proje dokümanını güncellemek için dosyayı yeniden yüklemeniz yeterli olacaktır.</td>
             </tr>
             <tr>
             ...
             <tr>
               <td colspan="2" valign="top"><ul>
                 <li class="icyazi">Yükleyeceğiniz dokümanın adında <strong>noktalama işaretleri</strong> bulunmamalıdır.</li>
                 <li class="icyazi">Yükleyeceğiniz dokümanın dosya uzantısı <strong>exe</strong>, <strong>com</strong>, <strong>bat</strong>, <strong>js</strong>, ve <strong>php</strong> olmamalıdır. (Yasaklanmış dosya uzantılarına sahip dosyaları sisteme yüklemek istiyorsanız bu dosyaları sıkıştırılmış dosya formatında sisteme yükleyebilirsiniz.)</li>
                 <li class="icyazi">Dokümanınızın dosya büyüklüğü <strong>2000 KB</strong>'ın üzerinde olmamalıdır.</li>
               </ul></td>
             </tr>
           </table>
           </form>
         </td>
         <td width="49%" valign="top"><table width="100%" border="0" align="right" cellpadding="3" cellspacing="2">
           <tr>
             <td colspan="4"><span class="baslik"><?php echo $row_ders['kod']; ?> - Ödevler &amp; Projeler</span></td>
             </tr>
           <tr>
           ...
           <?php if ($totalRows_prj == 0) { ?>
             <tr>
               <td colspan="4" class="icyazi">Sistemde bu ders için yüklü ödev / proje bulunmamaktadır.</td>
             </tr>
             <?php } ?>
				<?php if ($totalRows_prj > 0) { ?>
             <tr>
             <td width="61%" class="arabaslik">Doküman</td>
             <td width="33%" nowrap="nowrap" class="arabaslik">Teslim Tarihi</td>
             <td width="6%" colspan="2" class="arabaslik"><div align="center">İşlemler</div></td>
             </tr>
             <?php do { ?>
               <tr>
                 <td valign="top" class="icyazi"><a href="projeler/<?php echo $row_prj['dersno']; ?>_<?php echo $row_prj['dosyaadi']; ?>" target="_blank"><?php echo $row_prj['orjinalad']; ?></a></td>
                 <td valign="top" class="icyazi"><div align="center"><?php echo $row_prj['teslim']; ?></div></td>
                 <td width="7%" valign="top" class="icyazi"><div align="center"><a href="yonetim_ders_proje_sil.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."projeno=".$row_prj['no'] ?>"><img src="simgeler/del_16.gif" alt="Sil" width="16" height="16" border="0" onclick="GP_popupConfirmMsg('Doküman sistemden kalıcı olarak silinecektir!');return document.MM_returnValue" /></a></div></td>
                 <td width="7%" valign="top" class="icyazi"><div align="center"><a href="yonetim_ders_proje_guncelle.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."projeno=".$row_prj['no'] ?>"><img src="simgeler/edit_16.gif" alt="Teslim Tarihini G&uuml;ncelle" width="16" height="16" border="0" /></a></div></td>
               </tr>
