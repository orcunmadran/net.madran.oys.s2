...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE lms_proje SET teslim=%s WHERE `no`=%s",
                       GetSQLValueString($_POST['teslim'], "date"),
  ...
  $updateGoTo = "yonetim_ders_proje.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$deger_prj = "-1";
if (isset($_SESSION['MM_Username'])) {
  $deger_prj = $_SESSION['MM_Username'];
}
mysql_select_db($database_lmscon, $lmscon);
$query_prj = sprintf("SELECT P.* FROM lms_proje P, lms_ders D WHERE P.no = %s AND D.kimlik = %s AND P.dersno = D.no", GetSQLValueString($colname_prj, "int"),GetSQLValueString($deger_prj, "text"));
   ...
   <td rowspan="2" class="ustbaslik"><!-- InstanceBeginEditable name="Baslik" -->
     <table width="100%" border="0" cellspacing="5" cellpadding="5">
       <tr>
         ...
         <td width="93%">Ödev &amp; Proje Teslim Tarihi Güncelle</td>
       </tr>
     </table>
       ...
       <tr>
         <td width="60%" valign="top"><form action="<?php echo $editFormAction; ?>" method="POST" name="form1" id="form1">
           <table width="500" border="0" cellpadding="3" cellspacing="2">
             <tr>
               <td colspan="3" class="baslik"><?php echo $row_prj['orjinalad']; ?></td>
               </tr>
             <tr>
               <td colspan="3" class="icyazi"><hr size="1" /></td>
               </tr>
             <tr>
               <td width="99" class="icyazi"><strong>Teslim Tarihi</strong></td>
               <td width="4" class="icyazi"><strong>:</strong></td>
               <td width="342">                 <input name="teslim" type="text" class="metinkutusu" id="teslim" onclick="GetDate(this);" value="<?php echo $row_prj['teslim']; ?>" size="15" readonly="readonly" />               </td>
             </tr>
             <tr>
               <td><input name="no" type="hidden" id="no" value="<?php echo $row_prj['no']; ?>" /></td>
               <td>&nbsp;</td>
               <td><input name="button" type="submit" class="buton" id="button" onclick="YY_checkform('form1','teslim','#q','0','Teslim Tarihi');return document.MM_returnValue" value="Teslim Tarihini Güncelle" /></td>
             </tr>
           </table>
