...
if ((isset($_GET['projeno'])) && ($_GET['projeno'] != "")) {
  $deleteSQL = sprintf("DELETE FROM lms_proje WHERE `no`=%s",
                       GetSQLValueString($_GET['projeno'], "int"));

  mysql_select_db($database_lmscon, $lmscon);
  $Result1 = mysql_query($deleteSQL, $lmscon) or die(mysql_error());

  $deleteGoTo = "yonetim_ders_proje.php?dersno=".$_SESSION['dersno'];
...
mysql_select_db($database_lmscon, $lmscon);
$query_proje = sprintf("SELECT `no`, dersno, dosyaadi FROM lms_proje WHERE `no` = %s", GetSQLValueString($colname_proje, "int"));
