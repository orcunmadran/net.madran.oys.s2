...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE lms_kullanici SET sifre=%s, yetki=%s, ad=%s, soyad=%s, eposta=%s, web=%s WHERE kimlik=%s",
                       GetSQLValueString($_POST['sifre'], "text"),
                       GetSQLValueString($_POST['yetki'], "int"),
  ...
  $updateGoTo = "yonetim_kullanici_ara_goster.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_lmscon, $lmscon);
$query_yetkilendirme = "SELECT * FROM lms_seviye ORDER BY yetki ASC";
$yetkilendirme = mysql_query($query_yetkilendirme, $lmscon) or die(mysql_error());
$row_yetkilendirme = mysql_fetch_assoc($yetkilendirme);
$totalRows_yetkilendirme = mysql_num_rows($yetkilendirme);
...
if (isset($_GET['kimlik'])) {
  $deger_guncelle = $_GET['kimlik'];
}
mysql_select_db($database_lmscon, $lmscon);
$query_guncelle = sprintf("SELECT K.*, S.aciklama FROM lms_kullanici K, lms_seviye S WHERE K.yetki = S.yetki AND K.kimlik = %s", GetSQLValueString($deger_guncelle, "text"));
$guncelle = mysql_query($query_guncelle, $lmscon) or die(mysql_error());
...
<!-- fwtable fwsrc="lms_ic.png" fwpage="LMS ic" fwbase="lms_ic.gif" fwstyle="Dreamweaver" fwdocid = "52238024" fwnested="0" -->
  ...
  <tr>
   ...
   <td rowspan="2" class="ustbaslik"><!-- InstanceBeginEditable name="Baslik" -->
     <table width="100%" border="0" cellspacing="5" cellpadding="5">
       <tr>
         <td width="7%"><a href="javascript:history.back()"><img src="simgeler/back_24.gif" alt="Yönetim paneline geri dön" width="24" height="24" border="0" /></a></td>
         <td width="93%"><span class="ustbaslik">Kullanıcı İşlemleri</span></td>
         <td>&nbsp;</td>
       </tr>
     </table>
  ...
  <tr>
   ...
   <td colspan="19" valign="top"><!-- InstanceBeginEditable name="İcerik" -->
     <table width="100%" border="0" cellspacing="5" cellpadding="5">
       <tr>
         <td width="60%" valign="top"><form action="<?php echo $editFormAction; ?>" method="POST" name="form1" id="form1">
           <table width="100%" border="0" cellspacing="2" cellpadding="3">
             <tr>
               <td colspan="3" class="baslik">Kullanıcı Bilgilerini Güncelle</td>
             </tr>
             <tr>
               <td colspan="3" nowrap="nowrap" class="icyazi"><hr size="1" /></td>
             </tr>
             <tr>
               <td width="20%" nowrap="nowrap" class="icyazi"><strong>Kullanıcı Adı</strong></td>
               <td width="2%" class="icyazi"><strong>:</strong></td>
               <td width="78%"><label><span class="icyazi"><?php echo $row_guncelle['kimlik']; ?></span>
                   <input name="kimlik" type="hidden" id="kimlik" value="<?php echo $row_guncelle['kimlik']; ?>" />
               </label></td>
             </tr>
             <tr>
               <td nowrap="nowrap" class="icyazi"><strong>Şifre</strong></td>
               <td class="icyazi"><strong>:</strong></td>
               <td><input name="sifre" type="text" class="metinkutusu" id="sifre" value="<?php echo $row_guncelle['sifre']; ?>" size="20" maxlength="20" />
                   <span class="baslik">*</span></td>
             </tr>
             <tr>
               <td nowrap="nowrap" class="icyazi"><strong>Yetki</strong></td>
               <td class="icyazi"><strong>:</strong></td>
               <td><label>
                 <select name="yetki" class="metinkutusu" id="yetki">
                   <option value="<?php echo $row_guncelle['yetki']; ?>" selected="selected"><?php echo $row_guncelle['aciklama']; ?></option>
                   <option value="-----">-----</option>
                   <?php
do {  
?>
                   <option value="<?php echo $row_yetkilendirme['yetki']?>"><?php echo $row_yetkilendirme['aciklama']?></option>
                   <?php
} while ($row_yetkilendirme = mysql_fetch_assoc($yetkilendirme));
  $rows = mysql_num_rows($yetkilendirme);
  if($rows > 0) {
      mysql_data_seek($yetkilendirme, 0);
	  $row_yetkilendirme = mysql_fetch_assoc($yetkilendirme);
  }
?>
                 </select>
             ...
             <tr>
               ...
               <td class="icyazi"><strong>:</strong></td>
               <td><input name="ad" type="text" class="metinkutusu" id="ad" value="<?php echo $row_guncelle['ad']; ?>" size="30" maxlength="40" />
                   <span class="baslik">*</span></td>
             </tr>
             <tr>
               <td nowrap="nowrap" class="icyazi"><strong>Soyad</strong></td>
               <td class="icyazi"><strong>:</strong></td>
               <td><input name="soyad" type="text" class="metinkutusu" id="soyad" value="<?php echo $row_guncelle['soyad']; ?>" size="30" maxlength="40" />
                   <span class="baslik">*</span></td>
             </tr>
             <tr>
               <td nowrap="nowrap" class="icyazi"><strong>E - Posta</strong></td>
               <td class="icyazi"><strong>:</strong></td>
               <td><input name="eposta" type="text" class="metinkutusu" id="eposta" value="<?php echo $row_guncelle['eposta']; ?>" size="40" maxlength="50" />
                   <span class="baslik">*</span></td>
             </tr>
             <tr>
               <td nowrap="nowrap" class="icyazi"><strong>Web</strong></td>
               <td class="icyazi"><strong>:</strong></td>
               <td><input name="web" type="text" class="metinkutusu" id="web" value="<?php echo $row_guncelle['web']; ?>" size="40" maxlength="50" /></td>
             </tr>
             <tr>
               <td nowrap="nowrap" class="icyazi">&nbsp;</td>
               <td class="icyazi">&nbsp;</td>
               <td><label>
                 <input name="button" type="submit" class="buton" id="button" onclick="YY_checkform('form1','sifre','#q','0','Şifre','ad','#q','0','Ad','soyad','#q','0','Soyad','eposta','#S','2','E - Posta','kimlik','#q','0','Kullanıcı Adı');return document.MM_returnValue" value="Kullanıcı Bilgilerini Güncelle" />
               </label></td>
             </tr>
             <tr>
               <td colspan="2" nowrap="nowrap" class="icyazi"><div align="right"><span class="baslik">*</span></div></td>
         ...
         <td valign="top"><table width="100%" border="0" cellspacing="2" cellpadding="3">
           <tr>
             <td width="100%" class="baslik"><div align="center">Kullanıcı Ara</div></td>
           </tr>
           <tr>
             <td><hr size="1" /></td>
           </tr>
           <tr>
             <td><form id="form2" name="form2" method="get" action="yonetim_kullanici_ara_liste.php">
               <table border="0" align="center" cellpadding="3" cellspacing="2">
                 <tr>
                   <td nowrap="nowrap" class="icyazi"><strong>Anahtar Kelime</strong></td>
                   <td class="icyazi"><strong>:</strong></td>
                   <td class="icyazi"><label>
                     <input name="anahtar" type="text" class="metinkutusu" id="anahtar" size="20" maxlength="20" />
                   </label></td>
...
mysql_free_result($yetkilendirme);
