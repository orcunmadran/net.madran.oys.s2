...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE lms_duyuru SET baslik=%s, icerik=%s, baslama=%s, bitis=%s WHERE `no`=%s",
  ...
  $updateGoTo = "yonetim_sube_duyuru.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_lmscon, $lmscon);
$query_derskod = "SELECT no AS dersno,CONCAT( kod, ' - ', ad) AS dersbilgi FROM lms_ders ORDER BY kod ASC";
...
mysql_select_db($database_lmscon, $lmscon);
$query_duyuru = sprintf("SELECT DU.* FROM lms_duyuru DU WHERE DU.kimlik = %s AND DU.no = %s", GetSQLValueString($bir_duyuru, "text"),GetSQLValueString($iki_duyuru, "int"));
...
$icerik = new SpawEditor("icerik", $row_duyuru['icerik']);
?>
...
<!-- fwtable fwsrc="lms_ic.png" fwpage="LMS ic" fwbase="lms_ic.gif" fwstyle="Dreamweaver" fwdocid = "52238024" fwnested="0" -->
   ...
   <td rowspan="2" class="ustbaslik"><!-- InstanceBeginEditable name="Baslik" -->
     <table width="100%" border="0" cellspacing="5" cellpadding="5">
       <tr>
         ...
         <td width="93%">Duyuru İşlemleri</td>
       </tr>
     </table>
  ...
  <tr>
   ...
   <td colspan="19" valign="top"><!-- InstanceBeginEditable name="İcerik" -->
     <table width="100%" border="0" cellspacing="5" cellpadding="5">
       <tr>
         <td width="60%" class="baslik">Duyuru Güncelle</td>
         <td width="40%" rowspan="3" valign="top" class="baslik">&nbsp;</td>
       </tr>
       <tr>
         ...
         <td valign="top"><form action="<?php echo $editFormAction; ?>" id="form1" name="form1" method="POST">
           <table width="100%" border="0" cellspacing="2" cellpadding="3">
             <tr>
               ...
               <td><label><span class="icyazi"><?php echo $row_duyuru['hedef']; ?></span></label></td>
             </tr>
             <tr>
               <td nowrap="nowrap" class="icyazi"><strong>Başlık</strong></td>
               <td class="icyazi"><strong>:</strong></td>
               <td><label></label>
                   <input name="baslik" type="text" class="metinkutusu" id="baslik" value="<?php echo $row_duyuru['baslik']; ?>" size="55" maxlength="255" />
             ...
             <tr>
               ...
               <td><label>
                 <input name="baslama" type="text" class="metinkutusu" id="baslama" onclick="GetDate(this);" value="<?php echo $row_duyuru['baslama']; ?>" size="15" readonly="readonly" />
                 <span class="baslik">*</span></label></td>
             </tr>
             <tr>
               <td nowrap="nowrap" class="icyazi"><strong>Yayın Bitiş Tarihi</strong></td>
               <td class="icyazi"><strong>:</strong></td>
               <td><input name="bitis" type="text" class="metinkutusu" id="bitis" onclick="GetDate(this);" value="<?php echo $row_duyuru['bitis']; ?>" size="15" readonly="readonly" />
                 <span class="baslik">*</span></td>
               </tr>
             <tr>
  ...
  $icerik->show();
  ?>
                   <input name="no" type="hidden" id="no" value="<?php echo $row_duyuru['no']; ?>" /></td>
             </tr>
             <tr>
               <td colspan="3" class="icyazi"><label>
                 <input name="button" type="submit" class="buton" id="button" onclick="YY_checkform('form1','baslik','#q','0','Başlık','baslama','#q','0','Yayın Tarihi','bitis','#q','0','Yayın Bitiş Tarihi','kimlik','#q','0','Kullanıcı Adı');return document.MM_returnValue" value="Duyuru Güncelle" />
               </label></td>
             </tr>
           </table>
