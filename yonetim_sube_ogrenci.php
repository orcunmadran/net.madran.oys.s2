...
if (isset($_POST[$MM_flag])) {
  $MM_dupKeyRedirect="yonetim_sube_ogrenci_hata.php";
  $loginUsername = $_POST['kontrol'];
  $LoginRS__query = sprintf("SELECT kontrol FROM lms_sinif WHERE kontrol=%s", GetSQLValueString($loginUsername, "text"));
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO lms_sinif (subeno, kimlik, kontrol) VALUES (%s, %s, %s)",
  ...
  $insertGoTo = "yonetim_sube_ogrenci.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_lmscon, $lmscon);
$query_subebilgi = sprintf("SELECT S.no, CONCAT(D.kod, ' - ', D.ad) AS dersKodAd, S.kod, S.subeadi, CONCAT(K.ad, ' ', K.soyad) AS adsoyad FROM lms_sube S, lms_ders D, lms_kullanici K WHERE S.no = %s AND S.dersno = D.no AND S.kimlik = K.kimlik", GetSQLValueString($deger_subebilgi, "int"));
...
<!-- fwtable fwsrc="lms_ic.png" fwpage="LMS ic" fwbase="lms_ic.gif" fwstyle="Dreamweaver" fwdocid = "52238024" fwnested="0" -->
  ...
  <tr>
   ...
   <td rowspan="2" class="ustbaslik"><!-- InstanceBeginEditable name="Baslik" -->
     <table width="100%" border="0" cellspacing="5" cellpadding="5">
       <tr>
         <td width="7%"><a href="yonetim_subeler.php"><img src="simgeler/back_24.gif" alt="Şube işlemlerine geri dön" width="24" height="24" border="0" /></a></td>
         <td width="93%"><p>Öğrenci İşlemleri</p>
          </td>
         <td>&nbsp;</td>
       </tr>
     </table>
  ...
  <tr>
   ...
   <td colspan="19" valign="top"><!-- InstanceBeginEditable name="İcerik" -->
     <table width="100%" border="0" cellspacing="5" cellpadding="5">
       <tr>
         <td width="58%" valign="top"><form action="<?php echo $editFormAction; ?>" id="form1" name="form1" method="POST">
           <table width="100%" border="0" cellspacing="2" cellpadding="3">
             <tr>
               <td colspan="3" class="baslik">Öğrenci Ekle</td>
             </tr>
             <tr>
               <td colspan="3" class="icyazi"><hr size="1" /></td>
               </tr>
             <tr>
               <td width="28%" class="icyazi"><strong>Dersin Kodu ve Adı</strong></td>
               <td width="2%" class="icyazi"><strong>:</strong></td>
               <td width="70%" class="icyazi"><?php echo $row_subebilgi['dersKodAd']; ?></td>
             </tr>
             <tr>
               <td class="icyazi"><strong>Şube Kodu</strong></td>
               <td class="icyazi"><strong>:</strong></td>
               <td class="icyazi"><?php echo $row_subebilgi['kod']; ?></td>
             </tr>
             <tr>
               <td class="icyazi"><strong>Şube Sorumlusu</strong></td>
               <td class="icyazi"><strong>:</strong></td>
               <td class="icyazi"><?php echo $row_subebilgi['adsoyad']; ?></td>
             </tr>
             <tr>
               <td class="icyazi"><strong>Öğrenci Bilgisi</strong></td>
               <td class="icyazi"><strong>:</strong></td>
               <td class="icyazi"><label>
                 <select name="ogrenci" class="metinkutusu" id="ogrenci" onchange="MM_callJS('olustur()')">
                   <option value="VT">Seçiniz</option>
                   <?php
do {  
?>
                   <option value="<?php echo $row_ogrenciler['kimlik']?>"><?php echo $row_ogrenciler['bilgi']?></option>
                   <?php
} while ($row_ogrenciler = mysql_fetch_assoc($ogrenciler));
  $rows = mysql_num_rows($ogrenciler);
  if($rows > 0) {
      mysql_data_seek($ogrenciler, 0);
	  $row_ogrenciler = mysql_fetch_assoc($ogrenciler);
  }
?>
                 </select>
             ...
             <tr>
               <td><strong class="icyazi">Öğrenci No</strong></td>
               <td><strong>:</strong></td>
               <td><span class="icyazi">
                 <input name="kimlik" type="text" class="metinkutusu" id="kimlik" onchange="MM_callJS('olustur2()')" value="Öğrenci NO" size="20" maxlength="20" />
                 <span class="baslik">*</span></span></td>
             </tr>
             <tr>
               <td><input name="subeno" type="hidden" id="subeno" value="<?php echo $row_subebilgi['no']; ?>" />
                 <input name="subeadi" type="hidden" id="subeadi" value="<?php echo $row_subebilgi['subeadi']; ?>" />
                 <input type="hidden" name="kontrol" id="kontrol" /></td>
               <td>&nbsp;</td>
               <td><label>
                 <input name="button" type="submit" class="buton" id="button" onclick="YY_checkform('form1');return document.MM_returnValue" value="Öğrenci Ekle" />
               </label></td>
             </tr>
             <tr>
               <td colspan="2" valign="top"><div align="right"><span class="baslik">*</span></div></td>
               <td class="icyazi">Öğrenci ekleme işlemi esnasında &quot;Öğrenci Bilgisi&quot; bölümünden öğrenciyi seçebilir ya da &quot;Öğrenci No&quot; bölümünden direkt öğrenci numarasını girebilirsiniz.</td>
             </tr>
           </table>
              <input type="hidden" name="MM_insert" value="form1" />
         </form>
          <p>&nbsp;</p>           </td>
         <td width="42%" valign="top"><table width="100%" border="0" cellspacing="2" cellpadding="3">
           <tr>
             <td colspan="2" class="baslik">Sınıf Listesi</td>
             <td class="baslik"><div align="center"><a href="yonetim_sube_ogrenci_liste.php?<?php echo $MM_keepURL; ?>" target="_blank"><img src="simgeler/paste_24.gif" alt="Sınıf Listesi Al" width="24" height="24" border="0" /></a></div></td>
           </tr>
           <tr>
           ...
           <?php if ($totalRows_liste == 0) { ?>
             <tr>
               <td colspan="3" class="icyazi">Şubede kayıt öğrenci bulunmamaktadır. Sağ tarafta yer alan &quot;Öğrenci Ekle&quot; bölümünü kullanarak şubeye öğrenci ekleyebilirsiniz.</td>
             </tr>
             <?php } ?>
			 <?php if ($totalRows_liste > 0) { ?>
             <tr>
             <td width="25%" class="icyazi"><div align="left"><strong>Öğrenci No</strong></div></td>
             <td width="64%" class="icyazi"><strong>Ad Soyad</strong></td>
             <td width="11%" class="icyazi"><strong>İşlem</strong></td>
           </tr>
             <?php do { ?>
               <tr>
                  <td class="icyazi"><div align="left"><?php echo $row_liste['kimlik']; ?></div></td>
                 <td class="icyazi"><?php echo $row_liste['adsoyad']; ?></td>
                 <td><div align="center"><a href="yonetim_sube_ogrenci_sil.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."no=".$row_liste['no'] ?>"><img src="simgeler/del_16.gif" alt="Sil" width="16" height="16" border="0" onclick="GP_popupConfirmMsg('Öğrencinin şube kaydı silinecektir!');return document.MM_returnValue" /></a></div></td>
               </tr>
               <?php } while ($row_liste = mysql_fetch_assoc($liste)); ?>
             <tr>
               <td class="icyazi">&nbsp;</td>
               <td class="icyazi"><div align="right">Sınıf Mevcudu:</div></td>
               <td class="icyazi"><div align="center"><?php echo $totalRows_liste ?></div></td>
             </tr>
<?php } ?>
         </table></td>
       </tr>
