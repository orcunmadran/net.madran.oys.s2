...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE lms_sube_sohbet SET gun=%s, saat=%s, ad=%s, sifre=%s WHERE `no`=%s",
                       GetSQLValueString($_POST['gun'], "int"),
                       GetSQLValueString($_POST['saat'], "text"),
  ...
  $updateGoTo = "yonetim_sube_sohbet_belirle.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_lmscon, $lmscon);
$query_gunler = "SELECT gun, deger FROM lms_gunler ORDER BY `no` ASC";
$gunler = mysql_query($query_gunler, $lmscon) or die(mysql_error());
$row_gunler = mysql_fetch_assoc($gunler);
$totalRows_gunler = mysql_num_rows($gunler);

$veri_sohbet = "-1";
if (isset($_GET['sohbetno'])) {
  $veri_sohbet = $_GET['sohbetno'];
}
mysql_select_db($database_lmscon, $lmscon);
$query_sohbet = sprintf("SELECT GN.gun AS tamgun, SU.* FROM lms_sube_sohbet SU, lms_gunler GN WHERE SU.gun = GN.deger AND SU.no = %s", GetSQLValueString($veri_sohbet, "int"));
$sohbet = mysql_query($query_sohbet, $lmscon) or die(mysql_error());
...
<!-- fwtable fwsrc="lms_ic.png" fwpage="LMS ic" fwbase="lms_ic.gif" fwstyle="Dreamweaver" fwdocid = "52238024" fwnested="0" -->
  ...
  <tr>
   ...
   <td rowspan="2" class="ustbaslik"><!-- InstanceBeginEditable name="Baslik" -->
     <table width="100%" border="0" cellspacing="5" cellpadding="5">
       <tr>
         <td width="7%"><a href="yonetim_sube_sohbet.php?<?php echo $MM_keepURL; ?>"><img src="simgeler/back_24.gif" alt="Geri d&ouml;n" width="24" height="24" border="0" /></a></td>
         <td width="93%">Sohbet Gün ve Saatini Belirle</td>
       </tr>
     </table>
  ...
  <tr>
   ...
   <td colspan="19" valign="top"><!-- InstanceBeginEditable name="İcerik" -->
     <table width="100%" border="0" cellspacing="5" cellpadding="5">
       <tr>
         <td width="60%" class="baslik">Sohbet Gün ve Saatini Güncelle</td>
         <td width="40%" class="baslik">&nbsp;</td>
       </tr>
       <tr>
         <td valign="top"><form action="<?php echo $editFormAction; ?>" id="form1" name="form1" method="POST">
           <table width="100%" border="0" cellspacing="2" cellpadding="3">
             <tr>
               <td width="24%" nowrap="nowrap" class="icyazi"><strong>Gün</strong></td>
               <td width="3%" class="icyazi"><strong>:</strong></td>
               <td colspan="3" class="icyazi"><label>
                 <select name="gun" class="metinkutusu" id="gun">
                   <option selected="selected" value="<?php echo $row_sohbet['tamgun']; ?>"><?php echo $row_sohbet['tamgun']; ?></option>
                   <?php
do {  
?>
                   <option value="<?php echo $row_gunler['deger']?>"><?php echo $row_gunler['gun']?></option>
                   <?php
} while ($row_gunler = mysql_fetch_assoc($gunler));
  $rows = mysql_num_rows($gunler);
  if($rows > 0) {
      mysql_data_seek($gunler, 0);
	  $row_gunler = mysql_fetch_assoc($gunler);
  }
?>
                 </select>
             ...
             <tr>
               <td nowrap="nowrap" class="icyazi"><strong>Saat</strong></td>
               <td class="icyazi"><strong>:</strong></td>
               <td width="11%" class="icyazi"><label>
                 <input name="saat" type="text" class="metinkutusu" id="saat" value="<?php echo $row_sohbet['saat']; ?>" size="5" maxlength="5" />
               </label></td>
               <td width="4%" class="baslik">*</td>
               <td width="58%" class="icyazi">(Ör: 18:30)</td>
             </tr>
             <tr>
               <td nowrap="nowrap" class="icyazi"><strong>Oda Adı</strong></td>
               <td class="icyazi"><strong>:</strong></td>
               <td colspan="3" class="icyazi"><input name="ad" type="text" class="metinkutusu" id="ad" value="<?php echo $row_sohbet['ad']; ?>" size="20" maxlength="30" />
                 <span class="baslik">*</span></td>
             </tr>
             <tr>
               <td nowrap="nowrap" class="icyazi"><strong>Oda Şifresi</strong></td>
               <td class="icyazi"><strong>:</strong></td>
               <td colspan="3" class="icyazi"><label>
                 <input name="sifre" type="text" class="metinkutusu" id="sifre" value="<?php echo $row_sohbet['sifre']; ?>" size="20" maxlength="20" />
                 <span class="baslik">*</span></label></td>
             </tr>
             <tr>
               <td nowrap="nowrap" class="icyazi"><input name="no" type="hidden" id="no" value="<?php echo $row_sohbet['no']; ?>" /></td>
               <td class="icyazi">&nbsp;</td>
               <td colspan="3" class="icyazi"><label>
                 <input name="button" type="submit" class="buton" id="button" onclick="YY_checkform('form1','saat','#q','0','Saat','ad','#q','0','Ad','sifre','#q','0','Şifre');return document.MM_returnValue" value="Güncelle" />
               </label></td>
             </tr>
             <tr>
               <td colspan="2" nowrap="nowrap" class="icyazi"><div align="right"><span class="baslik">*</span></div></td>
               <td colspan="3" class="icyazi">Dolduruması zorunlu alanlar</td>
             </tr>
           </table>
