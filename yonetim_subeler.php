...
if (isset($_POST[$MM_flag])) {
  $MM_dupKeyRedirect="yonetim_sube_hata.php";
  $loginUsername = $_POST['subeadi'];
  $LoginRS__query = sprintf("SELECT subeadi FROM lms_sube WHERE subeadi=%s", GetSQLValueString($loginUsername, "text"));
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO lms_sube (kod, dersno, subeadi, kimlik) VALUES (%s, %s, %s, %s)",
  ...
  $insertGoTo = "yonetim_subeler.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_lmscon, $lmscon);
$query_oe = "SELECT kimlik, CONCAT(ad, ' ', soyad, ' - ', kimlik) AS eleman FROM lms_kullanici WHERE yetki IN (1,2,3) ORDER BY ad";
...
mysql_select_db($database_lmscon, $lmscon);
$query_s = sprintf("SELECT S.no, S.subeadi, D.kod AS derskodu, S.kod AS subekodu, CONCAT(K.ad,' ', K.soyad) AS adsoyad FROM lms_sube S, lms_ders D, lms_kullanici K WHERE D.kimlik = %s AND S.dersno = D.no AND S.kimlik = K.kimlik ORDER BY D.kod, S.kod", GetSQLValueString($deger_s, "text"));
       ...
       <tr>
         <td valign="top"><form id="form1" name="form1" method="POST" action="<?php echo $editFormAction; ?>">
           <table width="100%" border="0" cellspacing="2" cellpadding="3">
             <tr>
               <td colspan="3" class="baslik">Yeni Şube Ekle</td>
             </tr>
             <tr>
...
?>
             ...
             <tr>
               ...
               <td class="icyazi"><strong>:</strong></td>
               <td><input name="kod" type="text" class="metinkutusu" id="kod" onchange="MM_callJS('olustur()')" size="3" maxlength="3" /></td>
             </tr>

             <tr>
               <td><input type="hidden" name="subeadi" id="subeadi" /></td>
               <td>:</td>
               <td><label>
                 <input name="button" type="submit" class="buton" id="button" onclick="YY_checkform('form1','subeadi','#q','0','Şube adı','dersno','#q','1','Açılacak ders','kimlik','#q','1','Öğretim elemanı');return document.MM_returnValue" value="Yeni Şube Ekle" />
               </label></td>
             </tr>
           </table>
         ...
         <td valign="top"><table border="0" cellspacing="2" cellpadding="3">
           <tr>
             <td colspan="8" class="baslik">Sistemde Yer Alan Şubeleriniz</td>
             </tr>
           <tr>
             <td colspan="8"><hr size="1" /></td>
             </tr>
           <?php if ($totalRows_s == 0) {?>
           <tr>
             <td colspan="8" class="icyazi">Sistem içerisinde daha önce oluşturmuş olduğunuz bir şube bulunmamaktadır. Sol tarafta yer alan &quot;Yeni Şube Ekle&quot; bölümünden sisteme yeni bir şube ekleyebilirsiniz.</td>
             </tr>
             <?php }?>
             <?php if ($totalRows_s > 0) {?>
           <tr>
             <td nowrap="nowrap" class="icyazi"><strong>Şube</strong></td>
             <td nowrap="nowrap" class="icyazi"><strong>Ders Kodu</strong></td>
             <td nowrap="nowrap" class="icyazi"><strong>Şube</strong></td>
             <td nowrap="nowrap" class="icyazi"><strong>Öğretim Elemanı</strong></td>
             <td colspan="4" nowrap="nowrap" class="icyazi"><div align="center"><strong>İşlemler</strong></div></td>
             </tr>
           <?php do { ?>
             <tr>
               <td class="icyazi"><?php echo $row_s['subeadi']; ?></td>
               <td class="icyazi"><?php echo $row_s['derskodu']; ?></td>
               <td class="icyazi"><?php echo $row_s['subekodu']; ?></td>
               <td class="icyazi"><?php echo $row_s['adsoyad']; ?></td>
               <td width="1%"><div align="center"><a href="yonetim_sube_sil.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."no=".$row_s['no'] ?>"><img src="simgeler/del_16.gif" title="Sil" width="16" height="16" border="0" onclick="GP_popupConfirmMsg('&#350;ube bilgisi sistemden kal&#305;c&#305; olarak silinecektir!');return document.MM_returnValue" /></a></div></td>
               <td width="1%"><div align="center"><a href="yonetim_sube_guncelle.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."no=".$row_s['no'] ?>"><img src="simgeler/edit_16.gif" title="Güncelle" width="16" height="16" border="0" /></a></div></td>
               <td width="1%" bordercolor="0"><a href="yonetim_sube_ogrenci.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."subeno=".$row_s['no'] ?>"><img src="simgeler/user_16.gif" title="Öğrenci İşlemleri" width="16" height="16" border="0" /></a></td>
               <td width="1%" bordercolor="0"><a href="yonetim_sube_ogrenci_liste.php?<?php echo $MM_keepURL.(($MM_keepURL!="")?"&":"")."subeno=".$row_s['no'] ?>" target="_blank"><img src="simgeler/paste_16.gif" title="Sınıf listesini görüntüle" border="0" /></a></td>
             </tr>
